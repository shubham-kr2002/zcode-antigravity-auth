...
import { inferModelCapabilities, } from "../models/capabilities.js";
import { loadModelCache, } from "../models/discovery.js";
...
function getModelIdsForSetup() {
    ...
    if (cache && cache.models && Object.keys(cache.models).length > 0) {
        const ids = Object.keys(cache.models)
            ...
            if (/^(?:chat-bison|code-bison|chat_|test-|internal-|gemini-2\.5-pro$|gemini-3-pro$|gemini-3\.1-pro-high$|gemini-pro-agent$)/i.test(id))
                return false;
            return true;
        })
 ...
 */
function capabilitiesToZCodeConfig(caps) {
    const config = {
        limit: {
...
function writeZCodeConfig(config) {
    const path = getZCodeConfigPath();
...
export function setupAntigravityProvider() {
 ...
 */
export function removeAntigravityProvider() {
    const config = readZCodeConfig();
