...
import { z } from "zod";
import { DEFAULT_PROXY_PORT, DEFAULT_OAUTH_PORT, DEFAULT_RETRY_AFTER_SECONDS, DEFAULT_MAX_BACKOFF_SECONDS, DEFAULT_FAILURE_TTL_SECONDS, DEFAULT_MAX_RATE_LIMIT_WAIT_SECONDS, DEFAULT_QUOTA_REFRESH_INTERVAL_MINUTES, DEFAULT_SOFT_QUOTA_THRESHOLD_PERCENT, } from "./constants.js";
// ---- Config Schema ----
...
// ---- Runtime Config ----
let runtimeConfig = AntigravityConfigSchema.parse({});
export function initRuntimeConfig(partial) {
...
export function loadConfig() {
    ...
    if (process.env.ANTIGRAVITY_PROXY_PORT) {
        partial.proxy_port = Number.parseInt(process.env.ANTIGRAVITY_PROXY_PORT, 10);
    }
    if (process.env.ANTIGRAVITY_OAUTH_PORT) {
        partial.oauth_port = Number.parseInt(process.env.ANTIGRAVITY_OAUTH_PORT, 10);
    }
    // Debug flags
    ...
    // Quiet mode
    if (process.env.ANTIGRAVITY_QUIET === "1" ||
...
export function getManualAccessToken() {
    return process.env.ANTIGRAVITY_ACCESS_TOKEN || null;
}
export function getManualProjectId() {
