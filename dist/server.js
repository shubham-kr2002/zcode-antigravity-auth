...
import { transformNonStreamResponse, } from "./transform/response.js";
...
import { buildAntigravityUrl, detectRateLimit, getFailoverEndpoints, getCapacityBackoffDelay, sleep, } from "./api/retry.js";
import { discoverModels, buildModelRegistry, loadModelCache, } from "./models/index.js";
...
export function setAccountManager(mgr) {
    accountManager = mgr;
}
// ---- Logger ----
...
async function resolveAuth(model) {
    ...
    const selected = mgr.getCurrentOrNextForFamily(family, model, config.account_selection_strategy, "antigravity", config.pid_offset_enabled, config.soft_quota_threshold_percent, softQuotaCacheTtlMs);
    if (!selected) {
        throw new Error("No available accounts. All accounts are rate-limited or disabled. " +
    ...
    if (!accessToken || Date.now() >= (selected.expires ?? 0)) {
        try {
            const refreshed = await refreshAccessToken(selected.parts.refreshToken, selected.parts.projectId);
            ...
            throw new Error("Authentication failed. Run `antigravity-auth login` to re-authenticate.");
        }
    }
    // Determine header style based on model and rate limit state
    let headerStyle = "antigravity";
    if (family === "gemini") {
        ...
        if (!available) {
            ...
            if (mgr.hasOtherAccountWithAntigravityAvailable(selected.index, family, model)) {
                ...
                throw new Error("Current account rate-limited, retry to switch to another account.");
            }
 ...
 */
export async function initModelRegistry(getAccessToken) {
    // 1. Try loading from cache
    const cache = loadModelCache();
    if (cache) {
        const apiResponse = {
            models: Object.fromEntries(Object.entries(cache.models).map(([id, entry]) => [
                id,
...
function handleRateLimitError(res, response, bodyText, accountIndex, family, model) {
    ...
    mgrPromise.then((mgr) => {
        if (accountIndex >= 0) {
            ...
            if (account) {
                mgr.markRateLimitedWithReason(account, family, "antigravity", model, detection.reason, detection.retryAfterMs);
                mgr.requestSaveToDisk();
...
async function proxyStream(res, antigravityResponse) {
...
export function createApp() {
    ...
    app.get("/v1/accounts", async (_req, res) => {
        try {
            const mgr = await getAccountManager();
        ...
        try {
            ...
            while (true) {
                ...
                for (const endpointResult of endpoints) {
                    const url = buildAntigravityUrl(endpointResult.endpoint, isStreaming);
                    ...
                    for (let capRetry = 0; capRetry <= MAX_CAPACITY_RETRIES_PER_ENDPOINT; capRetry++) {
                        ...
                        try {
                            actualModelSent = JSON.parse(init.body).model ?? openaiReq.model;
                        }
                        catch { }
                        ...
                        if (antigravityResponse.ok) {
                            ...
                            const transformed = transformNonStreamResponse(responseText, openaiReq.model);
                        ...
                        if (antigravityResponse.status === 429 || antigravityResponse.status === 503 || antigravityResponse.status === 529) {
                            ...
                            if (auth.accountIndex >= 0) {
                                ...
                                const account = mgr.getAccounts().find(a => a.index === auth.accountIndex);
                                if (account) {
                                    mgr.markRateLimitedWithReason(account, family, auth.headerStyle, openaiReq.model, detection.reason, detection.retryAfterMs);
                                    mgr.requestSaveToDisk();
