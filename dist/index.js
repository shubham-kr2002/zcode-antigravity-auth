 ...
 */
import { loadConfig, initRuntimeConfig } from "./config.js";
...
async function main() {
    ...
    app.listen(port, () => {
        ...
        console.error(`[zcode-antigravity-proxy] Listening on http://127.0.0.1:${port}`);
        console.error(`[zcode-antigravity-proxy] Models: ${modelCount} models loaded from ${sourceLabel}`);
        console.error(`[zcode-antigravity-proxy] Health check: http://127.0.0.1:${port}/health`);
        console.error(`[zcode-antigravity-proxy] Set ANTIGRAVITY_ACCESS_TOKEN env var for manual auth, or run "antigravity-auth login"`);
    });
}
main().catch((err) => {
