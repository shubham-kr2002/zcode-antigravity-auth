...
import { parseRateLimitReason } from "../accounts/manager.js";
...
const globalRateLimitState = new Map();
function dedupKey(accountIndex, quotaKey) {
    return `${accountIndex}:${quotaKey}`;
}
function cleanupDedupState() {
    const now = Date.now();
 ...
 */
export function getDedupedBackoff(accountIndex, quotaKey, serverRetryAfterMs, maxBackoffMs = 60_000, baseDelay = FIRST_RETRY_DELAY_MS) {
    cleanupDedupState();
 ...
 */
export function resetRateLimitState(accountIndex, quotaKey) {
    const key = dedupKey(accountIndex, quotaKey);
    globalRateLimitState.delete(key);
}
/**
 ...
 */
export function resetAccountRateLimitState(accountIndex) {
...
export function detectRateLimit(response, bodyText) {
    const status = response.status;
    const message = bodyText ?? "";
    let reason = parseRateLimitReason(undefined, message, status);
    let retryAfterMs = retryAfterMsFromResponse(response);
    ...
    try {
        ...
        if (error) {
            ...
            reason = parseRateLimitReason(error.reason ?? error.status, error.message ?? message, error.code ?? status);
                    ...
                    if (detail["@type"]?.includes("QuotaFailure") && detail.violations) {
                        for (const violation of detail.violations) {
                            if (violation.retryDelay && retryAfterMs === null) {
                                retryAfterMs = parseDurationToMs(violation.retryDelay);
                            }
                        }
                    }
                    if (detail.retryDelay && retryAfterMs === null) {
                        retryAfterMs = parseDurationToMs(detail.retryDelay);
                    }
                }
            }
            // RetryInfo
 ...
 */
export function buildAntigravityUrl(endpoint, isStreaming) {
 ...
 */
export function getFailoverEndpoint(currentIndex, headerStyle) {
 ...
 */
export function getFailoverEndpoints(headerStyle) {
    const results = [];
