...
import { ANTIGRAVITY_SYSTEM_INSTRUCTION, CLAUDE_TOOL_SYSTEM_INSTRUCTION, getRandomizedHeaders, GEMINI_CLI_HEADERS, } from "../constants.js";
import { resolveModelWithTier, isClaudeModel } from "./model-resolver.js";
import { cleanJSONSchema, toGeminiSchema } from "./schema.js";
import { isClaudeThinkingModel } from "./claude.js";
import { isGemini3Model } from "./gemini.js";
import { sanitizeCrossModelPayloadInPlace } from "./cross-model-sanitizer.js";
// ---- Main Transform ----
export function transformRequest(openaiReq, options) {
    ...
    const toolCallNameById = new Map(); // call_id → function_name
    ...
    if (openaiReq.tools && openaiReq.tools.length > 0) {
        if (isClaude) {
            ...
            const functionDeclarations = openaiReq.tools
    ...
    if (isClaude) {
        ...
        if (tools && requestPayload.toolConfig) {
            const tc = requestPayload.toolConfig;
    ...
    const baseEndpoint = headerStyle === "gemini-cli"
