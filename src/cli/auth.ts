...
async function doLogin(): Promise<void> {
  ...
  console.log(
    "This will open Google's OAuth consent screen in your browser.",
  );
  console.log(
    "After approving, you'll be redirected back and authenticated.\n",
  );
  ...
  if (useManual) {
    ...
    console.log(
      `✅ Authenticated${result.email ? ` as ${result.email}` : ""}!`,
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      "⚠ Could not start callback server. Falling back to manual flow.",
    );
...
function doSetup(): void {
  ...
  const result: SetupResult = setupAntigravityProvider();
  ...
  console.log(
    "   1. Start the proxy:   antigravity-auth start",
  );
  console.log(
    "   2. Restart ZCode to pick up the new provider\n",
  );
}
...
async function doInstall(): Promise<void> {
  ...
  if (accounts.length === 0) {
    ...
    console.log(
      `✅ Found ${accounts.length} authenticated account(s).`,
    );
  ...
  console.log(
    `   Then restart ZCode to use Antigravity models.\n`,
  );
}
...
function doUninstall(): void {
  ...
  if (removed) {
    ...
    console.log(
      "   Accounts are preserved in ~/.zcode/antigravity-accounts.json",
    );
    console.log(
      "   Run `antigravity-auth setup` to re-add it later.\n",
    );
  } else {
