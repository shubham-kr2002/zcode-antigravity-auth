...
import { loadAccounts, saveAccounts, type StoredAccount, type AccountStorage, type ModelFamily, type CooldownReason, type RateLimitStateV3 } from "./storage.js";
import {
  getHealthTracker,
  getTokenTracker,
  selectHybridAccount,
...
import { getModelFamily, resolveQuotaGroup } from "../api/model-resolver.js";
...
// ---- Refresh Parts (simplified from reference) ----
...
function isRateLimitedForHeaderStyleInternal(
...
export class AccountManager {
        ...
        .map((acc, index): ManagedAccount | null => {
          ...
          return {
            ...
            fingerprintHistory: acc.fingerprintHistory?.map(h => ({
              fingerprint: h.fingerprint as Fingerprint,
  ...
  removeAccountByIndex(accountIndex: number): boolean {
    if (accountIndex < 0 || accountIndex >= this.accounts.length) return false;
    const account = this.accounts[accountIndex];
    if (!account) return false;
