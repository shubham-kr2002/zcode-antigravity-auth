 ...
 */
function stripUnsupportedFields(schema: Record<string, unknown>): Record<string, unknown> {
...
export function cleanJSONSchema(
  ...
  for (const [key, value] of Object.entries(schema)) {
    if (BASIC_UNSUPPORTED_KEYS.has(key)) continue;
 ...
 */
export function createPlaceholderSchema(): Record<string, unknown> {
  return {
    type: "object",
    properties: {
      [EMPTY_SCHEMA_PLACEHOLDER_NAME]: {
