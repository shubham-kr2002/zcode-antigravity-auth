...
import {
  ...
  supportsThinkingTiers as inferSupportsThinkingTiers,
  hasPreSuffixedTier,
...
let _registry: ModelRegistryData = {
  models: [],
  nameMap: {},
  capabilities: {},
  aliases: {},
 ...
 */
export function setModelRegistryForResolution(registry: ModelRegistryData): void {
  _registry = registry;
}
...
export const MODEL_ALIASES: Record<string, string> = new Proxy({} as Record<string, string>, {
  get(_target, prop: string) {
    const registry = getRegistry();
    // Check registry aliases first (auto-discovered)
    if (registry.aliases[prop] !== undefined) return registry.aliases[prop];
...
const FALLBACK_MODEL_NAME_MAP: Record<string, string> = {
  // Claude models
...
function extractThinkingTierFromModel(model: string): ThinkingTier | undefined {
  if (!supportsThinkingTiers(model)) return undefined;
...
export function resolveModelWithTier(
  ...
  const preferGeminiCli =
    ...
    !isImageModel &&
    !isClaudeModel;
  const quotaPreference: HeaderStyle = preferGeminiCli
    ? "gemini-cli"
    : "antigravity";
  ...
  const budgetKey: "low" | "medium" | "high" =
    tier === "minimal" || tier === "extra-low" ? "low" : tier as "low" | "medium" | "high";
 ...
 */
export function resolveAntigravityModel(modelName: string): string {
