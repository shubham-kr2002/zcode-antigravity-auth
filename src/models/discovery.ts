...
import { inferModelCapabilities, type ModelCapabilities } from "./capabilities.js";
...
export function loadModelCache(): ModelCache | null {
  ...
  try {
    ...
    if (!cache.updatedAt || !cache.models) return null;
...
export function saveModelCache(models: Record<string, { displayName?: string }>): void {
  const path = getCachePath();
  const dir = join(path, "..");
  if (!existsSync(dir)) {
...
export async function fetchAvailableModelsFromAPI(
  accessToken: string,
  projectId: string,
): Promise<FetchAvailableModelsResponse> {
  const endpoint = ANTIGRAVITY_ENDPOINT_PROD;
  const quotaUserAgent =
    getAntigravityHeaders()["User-Agent"] || "antigravity/windows/amd64";
...
const THINKING_TIERS = ["minimal", "extra-low", "low", "medium", "high"] as const;
...
export function buildModelRegistry(
  apiResponse: FetchAvailableModelsResponse,
): ModelRegistryData {
  const models: OpenAIModelEntry[] = [];
  const nameMap: Record<string, string> = {};
  const capabilities: Record<string, ModelCapabilities> = {};
  const aliases: Record<string, string> = {};
  const preSuffixedModels = new Set<string>();
  ...
  for (const modelId of modelIds) {
    ...
    const cap: ModelCapabilities | undefined = capabilities[publicName];
    if (!cap?.reasoning?.enabled) continue;

    const variants: readonly string[] = cap.reasoning.variants;
    for (let ti = 0; ti < variants.length; ti++) {
      const tier: string = variants[ti]!;
      // Skip if tier is already in the model name
      if (publicName.endsWith(`-${tier}`)) continue;
  ...
  for (const [modelId, cap] of Object.entries(capabilities)) {
    ...
    if (lower.includes("thinking")) {
      for (const tier of THINKING_TIERS) {
        ...
        if (tier === "minimal" || tier === "extra-low") continue;
...
export async function discoverModels(
  ...
  if (cache) {
    ...
    const syntheticResponse: FetchAvailableModelsResponse = {
      models: {},
    };
  ...
  try {
    ...
    const cacheData: Record<string, { displayName?: string }> = {};
    ...
    if (apiResponse.models) {
      saveModelCache(
        Object.fromEntries(
          Object.entries(apiResponse.models).map(([id, entry]) => [
            id,
            { displayName: entry.displayName },
          ]),
        ),
